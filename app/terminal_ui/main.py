...
def _render_result(result: CommandRun) -> None:
    table = Table(show_header=False)
    table.add_row("Run ID", result.metadata.id)
    table.add_row("Timestamp", result.metadata.timestamp.isoformat())
    table.add_row("Source cmd", result.metadata.source_text or "<empty>")
    table.add_row("Normalized cmd", result.metadata.command_en or "<empty>")
    table.add_row("Language", result.metadata.language or "<unknown>")
    table.add_row("Target language", result.metadata.target_language)
    table.add_row("Normalization", result.metadata.normalization_status)
    table.add_row("PCS", result.metadata.pcs_status)
    ...
    table.add_row("Run dir", result.artifacts.run_dir)
    console.print(Panel(table, title="Command Result"))
...
@app.command("normalize-text")
def normalize_text(
    text: str = typer.Argument(..., help="Direct source command text."),
    language: str | None = typer.Option(default=None, help="Optional language override."),
) -> None:
    """Normalize direct text input into the English command pipeline"""

    try:
        result = build_app_context().service.normalize_text_input(text, language=language)
    except Exception as error:
        console.print(f"[red]Normalization failed:[/red] {error}")
        raise typer.Exit(code=1) from error
