...
def _choose_audio_source() -> Any | None:
    source = st.radio(
        "Source",
        options=["Record with microphone", "Upload WAV file"],
        horizontal=True,
    )

    if source == "Record with microphone":
        return st.audio_input("Record a voice command")

    return st.file_uploader("Upload a WAV file", type=["wav"])


def main() -> None:
    st.set_page_config(page_title="Smart Voice Kit", layout="centered")

    context = get_context()
    service = context.service

    st.title("Smart Voice Kit")
    ...
    if st.button("Transcribe recording", type="primary", use_container_width=True):
        try:
            with st.spinner("Running local ASR..."):
