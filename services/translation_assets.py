...
from schemas.config import TranslationSettings
...
class TransformersTranslationAssetPreparer:
    def __init__(self, settings: TranslationSettings, console: Console | None = None) -> None:
    ...
    def prepare(self, force_download: bool = False) -> ModelPreparationResult:
        if self.settings.model_path is not None:
            if not self.settings.model_path.exists():
                raise FileNotFoundError(
                    f"Configured translation model_path does not exist: {self.settings.model_path}"
                )
        ...
        if self.settings.download_root is None:
            raise ValueError("Translation download_root must be configured for model preparation.")
        ...
        if not downloadable_infos and not force_download:
            self.console.print("[green]Translation model is already cached locally.[/green]")
            return ModelPreparationResult(
                task="translation",
                family=self.settings.family,
                provider=self.settings.provider,
